...
This removes any effect sizes above 10 or below -10. The plot above shows only the experiments that have outlying effect sizes not the specific effect size itself.
...
# Check of PD
  ...
  sdata <- readRDS(here("output", "Output data", "multivariate",  "mlma_sa_data.rds"))
}
```
...
data_warm2  %>%  group_by(Paper.code)  %>% reframe(es_repro = mean(es_reproduction), es_long = mean(es_longevity), 
  mean_v = (v_reproduction + v_longevity)/2) %>% ggplot(aes(x = es_repro, y = es_long, size = mean_v)) + 
  geom_point(alpha = 1/10, show.legend = FALSE) + 
  scale_size(range = c(2, 15)) +
  labs(x = "Average effect size (Reproduction)", y = "Average effect size (Adult lifespan)") +
  theme_bw() +
...
data_cool2  %>%  group_by(Paper.code)  %>% reframe(es_repro = mean(es_reproduction), es_long = mean(es_longevity), 
  mean_v = (v_reproduction + v_longevity)/2) %>% ggplot(aes(x = es_repro, y = es_long, size = mean_v)) + 
  geom_point(alpha = 1/10, show.legend = FALSE) + 
  scale_size(range = c(2, 15)) +
  labs(x = "Average effect size (Reproduction)", y = "Average effect size (Adult lifespan)") +
