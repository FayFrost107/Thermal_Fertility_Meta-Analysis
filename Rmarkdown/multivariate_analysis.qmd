...
# Check of PD
  ...
  saveRDS(mlma_sa, here("output", "models", "mlma_sa_warm.rds"))
...
table <- data.frame(Parameters = c("Longevity (mean)", "Reproduction (mean)", "Linear Treatment Temperature (centered) - Longevity", "Linear Treatment Temperature (centered) - Reproduction"),
                    Est. = round(mlma_sa_warm$b, 4),
                    `L 95% CI` = round(mlma_sa_warm$ci.lb, 4),
                    `U 95% CI` = round(mlma_sa_warm$ci.ub, 4),
                    `df` = round(mlma_sa_warm$ddf,2),
                    `p-value` = sapply(round(mlma_sa_warm$pval,4), function(x) p_value(x)), row.names = NULL, check.names = FALSE)
flextable(table)
```
...
# Check of PD
  ...
  saveRDS(mlma_sa, here("output", "models", "mlma_sa_cool.rds"))
...
table <- data.frame(Parameters = c("Longevity (mean)", "Reproduction (mean)", 
                                   "Linear Treatment Temperature (centered) - Longevity", "Linear Treatment Temperature (centered) - Reproduction",  
                                   "Quadratic Treatment Temperature (centered) - Longevity",   "Quadratic Treatment Temperature (centered) - Reproduction"),
                    Est. = round(c(mlma_sa_cool$b[1:2],mv_mlma_split.cool_quad.report$b[1:4]), 4),
                    `L 95% CI` = round(c(mlma_sa_cool$ci.lb[1:2],mv_mlma_split.cool_quad.report$ci.lb[1:4]), 4),
                    `U 95% CI` = round(c(mlma_sa_cool$ci.ub[1:2],mv_mlma_split.cool_quad.report$ci.ub[1:4]), 4),
                    `df` = round(c(mlma_sa_cool$ddf[1:2],mv_mlma_split.cool_quad.report$ddf[1:4]),2),
                    `p-value` = sapply(round(c(mlma_sa_cool$pval[1:2],mv_mlma_split.cool_quad.report$pval[1:4]),4), function(x) p_value(x)), row.names = NULL, check.names = FALSE)
flextable(table)
```
