...
## Treatment temperature
```{r, eval=TRUE}
meta_trait_treattemp_new <- rma.mv(es, new_vcv,  mod= ~treattemp, random= list(~ 1|study_code,  ~1|obs), data= rdata_exc205, method= "REML")
```
...
## Warm vs Cool
```{r, eval=TRUE}
meta_trait_warm_new <- rma.mv(es, new_vcv,  mod= ~warm.cool, random= list(~ 1|study_code,  ~1|obs), data= rdata_exc205, method= "REML")
```
...
## Difference
```{r, eval=TRUE}
meta_trait_diff_new <- rma.mv(es, new_vcv,  mod= ~diff, random= list(~ 1|study_code,  ~1|obs), data= rdata_exc205, method= "REML")
```
...
```{r, eval=TRUE}
meta_trait_treat2_new <- rma.mv(es, new_vcv,  mod= ~ poly(treattemp, degree=2, raw=TRUE), random= list(~ 1|study_code,  ~1|obs), data= rdata_exc205, method= "REML")
```
...
```{r, eval=TRUE}
meta_trait_bintemp_new <- rma.mv(es, new_vcv,  mod= ~bin.temp-1,  random= list(~ 1|study_code,  ~1|obs), data= rdata_exc205, method= "REML")
...
```{r, eval=FALSE, echo=TRUE}
meta_sa_treat <- rma.mv(es, VCV_shared_sa,  mod= ~treattemp, random= list(~ 1|study_code,  ~1|obs), data= sdata, method= "REML")
```
