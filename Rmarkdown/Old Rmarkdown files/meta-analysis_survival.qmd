...
* **treattemp**: The treatment temperature, which we expect to have a non-linear relationship to survival.
...
```{r, eval=TRUE}
meta_trait_treattemp_sa <- rma.mv(es, new_vcv,  mod= ~treattemp, random= list(~ 1|study_code,  ~1|obs), test = "t", dfs = "contain", data= rdata_exc205, method= "REML")
```
...
```{r, eval=TRUE}
meta_trait_treat2_sa <- rma.mv(es, new_vcv,  mod= ~ poly(treattemp, degree=2, raw=TRUE), random= list(~ 1|study_code,  ~1|obs), test = "t", dfs = "contain", data= rdata_exc205, method= "REML")
```
