...
# Using this matrix, we can now capture the shared control. here, we now just feed in the V matrix we just created and the shared control column. This will then create a new V matrix with the shared control column as the cluster.
   V <- make_VCV_matrix(data = data_long_final, matrix = V_1, cluster = "shared_control",  V = "v", rho = 0.5)
...
# Ok, but this doesn't quite get us there the whole way because we need to try and capture this shared refence temperture control. vcalc doesn't seem to be able to so two seperate groupings at once. Thought we could do this with "type" and "obs" arguments specified but for some reason it still calculates the covariance between logenvity and reproduction and we really only want the covariance between like outcomes. So, we'll have to do this manually.

 # Find the rows in teh shared control column that match
test <- data_long_final  %>% filter(Experiment.code == "HUM148A")
