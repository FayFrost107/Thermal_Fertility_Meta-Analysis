...
ggplot(data = pest_rep_data, aes(x = c_treattemp, y = pred, col = Agricultural.importance, linetype = Agricultural.importance)) +
  ...
  theme_bw() + 
    coord_cartesian(ylim = c(-10,1))+
  labs(title = "Predicted temperature effect on reproduction",
       x = "Observed temperature deviation from 25C",
       y = "Effect size",
...
sdata <- subset(new_data, es > minq & es < maxq)
saveRDS(sdata, here("Output", "Output data", "reproduction", "pest_rep_sa_data.rds"))
...
# re-run quadratic model
  ...
  saveRDS(mlma_sa_pest.fixed, here("output", "models", "reproduction", "mlma_sa_pest.fixed.rds"))
  saveRDS(sdata, here("output", "Output data", "reproduction", "pest_rep_sa_data.rds"))
} else {
  mlma_sa_pest.fixed <- readRDS(here("output", "models", "reproduction", "mlma_sa_pest.fixed.rds"))
  pest_rep_sa_data <- readRDS(here("Output", "Output data", "reproduction", "pest_rep_sa_data.rds"))
}
