...
sex_data$Sex.exposed[which(sex_data$Sex.exposed == "Parthenogenetic")] <- "Female"

sex_data <- subset(sex_data, Sex.exposed != "Male")
sex_data <- subset(sex_data, Sex.exposed != "Unsure")

# Save lumped sex category data
saveRDS(sex_data, here("Output", "Output data", "reproduction", "data_rep_sex.rds"))

# Lump life stage categories so that we have cases where only adults were exposed (Adult), 
...
# Save lumped life stage data
saveRDS(ls_data, here("Output", "Output data", "reproduction", "data_rep_ls.rds"))

if(rerun){
VCV_shared_sex <- impute_covariance_matrix(vi=sex_data$v, cluster = sex_data$shared_control, r=0.5)


meta_treat_sex <- rma.mv(es, VCV_shared_sex,  mod= ~poly(c_treattemp, degree=2, raw=TRUE)*Sex.exposed, 
                         random= list(~ 1|study_code,  ~1|obs), data= sex_data, method= "REML", test="t")
...
saveRDS(meta_treat_ls_nointer, here("Output", "models", "reproduction", "meta_treat_rep_ls_nointer.rds"))
  ...
  sex_data <- readRDS(here("Output", "Output data", "reproduction", "data_rep_sex.rds"))
  ...
  ls_data <- readRDS(here("Output", "Output data", "reproduction", "data_rep_ls.rds"))
  }
...
# Remove outliers
sa_rep_sexdata <- subset(sex_data, es > -10 & es < 10) 
saveRDS(sa_rep_sexdata, here("Output", "Output data", "reproduction", "sensitivity_data_rep_sex.rds"))

if(rerun){

  
# re-compute the covariance matrix for subsetted data
VCV_shared_sa <- impute_covariance_matrix(vi=sa_rep_sexdata$v, cluster = sa_rep_sexdata$shared_control, r=0.5)

# re-run quadratic model
meta_sa_sex <- rma.mv(es, VCV_shared_sa,  mod= ~ poly(c_treattemp, degree=2, raw=TRUE) * Sex.exposed, 
                         random= list(~ 1|study_code,  ~1|obs), data= sa_rep_sexdata, method= "REML", test="t")
...
```{r, tbl-sasex}
#| label: tbl-sasex
#| tbl-cap: Non-linear effects of temperature and sex exposed on mean effect size of reproduction with most extreme effect sizes removed. 
table <- data.frame(Parameters = c("Linear Treatment Temperature (centered) - Both","Quadratic Treatment Temperature (centered) - Both", "Both (mean)", "Females only (mean)", "Treatment Temperature (centered) - Females only","Quadratic Treatment Temperature (centered) - Females only"),
...
sa_rep_ls_data <- subset(ls_data, es > -10 & es < 10) # Removes 58 (negative) data points
saveRDS(sa_rep_ls_data, here("Output", "Output data", "reproduction", "sa_rep_ls_data.rds"))


if(rerun){

# re-compute the covariance matrix for subsetted data
VCV_shared_sa <- impute_covariance_matrix(vi=sa_rep_ls_data$v, cluster = sa_rep_ls_data$shared_control, r=0.5)

# re-run quadratic model
meta_sa_ls <- rma.mv(es, VCV_shared_sa,  mod= ~ poly(c_treattemp, degree=2, raw=TRUE) * Life.stage.of.animal, 
                         random= list(~ 1|study_code,  ~1|obs), data= sa_rep_ls_data, method= "REML", test="t")
...
```{r, tbl-sals}
#| label: tbl-sals
#| tbl-cap: Non-linear effects of temperature and life stage of animal on mean effect size of reproduction after removing the 10% most extreme effect sizes.
table <- data.frame(Parameters = c("Linear Treatment Temperature (centered) - Adult","Quadratic Treatment Temperature (centered) - Adult", "Adult (mean)", "Immature (mean)", "Treatment Temperature (centered) - Immature","Quadratic Treatment Temperature (centered) - Immature"),
...
sa_rep_ls_data$pred <- preds.rep.ls$pred  
sa_rep_ls_data$pred.lb <- preds.rep.ls$pi.lb
sa_rep_ls_data$pred.ub <- preds.rep.ls$pi.ub
sa_rep_ls_data$c.lb <- preds.rep.ls$ci.lb
sa_rep_ls_data$c.ub <- preds.rep.ls$ci.ub

library(ggplot2)

ggplot(data = sa_rep_ls_data, aes(x = c_treattemp, y = pred, col = Life.stage.of.animal, linetype = Life.stage.of.animal)) +
  geom_ribbon(aes(ymin = c.lb, ymax = c.ub, fill = Life.stage.of.animal), alpha = 0.15) +
  geom_line() +
  scale_color_manual(values = c("purple", "orange")) +  # Set colors for the points and lines
  scale_fill_manual(values = c("purple", "orange")) +    # Set colors for the ribbons
...
saveRDS(sdata, here("output", "Output data", "reproduction", "data_rep_sa_pest.rds"))
  ...
  data_rep_sa_pest<- readRDS(here("Output", "Output data", "reproduction", "data_rep_sa_pest.rds"))
}
...
preds.rep.sa.pest <- predict(mlma_sa_pest, addx=TRUE)

data_rep_sa_pest$pred <- preds.rep.sa.pest$pred  
data_rep_sa_pest$pred.lb <- preds.rep.sa.pest$pi.lb
data_rep_sa_pest$pred.ub <- preds.rep.sa.pest$pi.ub
data_rep_sa_pest$c.lb <- preds.rep.sa.pest$ci.lb
data_rep_sa_pest$c.ub <- preds.rep.sa.pest$ci.ub

library(ggplot2)

ggplot(data = data_rep_sa_pest, aes(x = c_treattemp, y = pred, col = Agricultural.importance, linetype = Agricultural.importance)) +
  geom_ribbon(aes(ymin = c.lb, ymax = c.ub, fill = Agricultural.importance), alpha = 0.15) +
  geom_line() +
  scale_color_manual(values = c("purple", "orange")) +  # Set colors for the points and lines
  scale_fill_manual(values = c("purple", "orange")) +    # Set colors for the ribbons
...
#| fig-cap: Bubble plot of effect size of reproduction when controlling for temperature with < 24 hour exposure observations removed. 
...
sex_data <- readRDS(here("Output", "Output data", "reproduction", "data_rep_sex.rds"))
chronic_exposure_data <- sex_data

chronic_exposure_data <- subset(chronic_exposure_data, Exposure.duration != "< 24 hours")
chronic_exposure_data <- subset(chronic_exposure_data, Exposure.duration != "1 to 5 days")


if(rerun){

VCV_shared_sex <- impute_covariance_matrix(vi=chronic_exposure_data$v, cluster = chronic_exposure_data$shared_control, r=0.5)

meta_sex_chronic <- rma.mv(es, VCV_shared_sex,  mod= ~poly(c_treattemp, degree=2, raw=TRUE)*Sex.exposed, 
                         random= list(~ 1|study_code,  ~1|obs), data= chronic_exposure_data, method= "REML", test="t")

saveRDS(meta_sex_chronic, here("Output", "models", "reproduction", "meta_sex_chronic.rds"))
saveRDS(chronic_exposure_data, here("Output", "Output data", "reproduction", "chronic_exposure_data.rds"))
...
```{r, fig-figsexsa2}
#| label: fig-figsexsa2
#| fig-cap: Predicted models of sex exposed and temperature effects on reproduction (chronic exposure only).  

preds.rep.sex <- predict(meta_sex_chronic, addx=TRUE)


chronic_exposure_data$pred <- preds.rep.sex$pred  
chronic_exposure_data$pred.lb <- preds.rep.sex$pi.lb
chronic_exposure_data$pred.ub <- preds.rep.sex$pi.ub
chronic_exposure_data$c.lb <- preds.rep.sex$ci.lb
chronic_exposure_data$c.ub <- preds.rep.sex$ci.ub

library(ggplot2)

ggplot(data = chronic_exposure_data, aes(x = c_treattemp, y = pred, col = Sex.exposed, linetype = Sex.exposed)) +
  geom_ribbon(aes(ymin = c.lb, ymax = c.ub, fill = Sex.exposed), alpha = 0.15) +
  geom_line() +
  scale_color_manual(values = c("purple", "orange")) +  # Set colors for the points and lines
  scale_fill_manual(values = c("purple", "orange")) +    # Set colors for the ribbons
  ...
  coord_cartesian(ylim = c(-10,1)) +
  theme_bw() + 
  labs(title = "Predicted temperature effect on reproduction for chronic exposure",
       x = "Experimental temperature (°C)",
       y = "Standardised change in reproduction",
       color = "Sex exposed",  # Set the title of the color legend
       fill = "Sex exposed",
...
## Chronic exposure only SA with lifestage of animal grouped into 2 categories (adult and immature)

chronic_exposure_ls_data <- ls_data

chronic_exposure_ls_data <- subset(chronic_exposure_ls_data, Exposure.duration != "< 24 hours")
chronic_exposure_ls_data <- subset(chronic_exposure_ls_data, Exposure.duration != "1 to 5 days")

if(rerun){

VCV_shared_ls <- impute_covariance_matrix(vi=chronic_exposure_ls_data$v, cluster = chronic_exposure_ls_data$shared_control, r=0.5)

meta_ls_chronic <- rma.mv(es, VCV_shared_ls,  mod= ~poly(c_treattemp, degree=2, raw=TRUE)*Life.stage.of.animal, 
                         random= list(~ 1|study_code,  ~1|obs), data= chronic_exposure_ls_data, method= "REML", test="t")

saveRDS(meta_ls_chronic, here("Output", "models", "reproduction", "meta_ls_chronic.rds"))
saveRDS(chronic_exposure_ls_data, here("Output", "Output data", "reproduction", "chronic_exposure_ls_data.rds"))
...
## data with removed outlying es
sdata <- readRDS(here("Output", "Output data", "reproduction", "data_rep_sa.rds"))
VCV_shared_sa <- impute_covariance_matrix(vi=sdata$v, cluster = sdata$shared_control, r=0.5)
...
rep_data_removing_assumed_rearing_temp <- rdata[!is.na(rdata$Rearing.temperature),]

if(rerun){
VCV_shared <- impute_covariance_matrix(vi=rep_data_removing_assumed_rearing_temp$v, cluster = rep_data_removing_assumed_rearing_temp$shared_control, r=0.5)

# treat temp^2 centered
meta_trait_treat2_remove_rear <- rma.mv(es, VCV_shared,  mod= ~poly(c_treattemp, degree=2, raw=TRUE), random= list(~ 1|study_code,  ~1|obs), data= rep_data_removing_assumed_rearing_temp, method= "REML")

r2_ml(meta_trait_treat2_remove_rear)

saveRDS(meta_trait_treat2_remove_rear, here("Output", "models", "reproduction",  "mv_rep_ctemp2_remove_rear.rds"))
}else{
  meta_trait_treat2_remove_rear  <- readRDS(here("Output", "models", "reproduction", "mv_rep_ctemp2_remove_rear.rds"))
}
```
...
#| tbl-cap: Non-linear effects of temperature on mean effect size of reproduction (Sensitivity analysis removing assumed rearing temperatures)
table <- data.frame(Parameters = c("reproduction (mean)", "Linear Treatment Temperature (centered) - reproduction","Quadratic Treatment Temperature (centered) - reproduction"),
...
rep_data_removing_aquatic <- rdata[rdata$Habitat == "Terrestrial",]

if(rerun){

VCV_shared <- impute_covariance_matrix(vi=rep_data_removing_aquatic$v, cluster = rep_data_removing_aquatic$shared_control, r=0.5)

# treat temp^2 centered
meta_trait_treat2_remove_aquatic <- rma.mv(es, VCV_shared,  mod= ~poly(c_treattemp, degree=2, raw=TRUE), random= list(~ 1|study_code,  ~1|obs), data= rep_data_removing_aquatic, method= "REML")

r2_ml(meta_trait_treat2_remove_aquatic)

saveRDS(meta_trait_treat2_remove_aquatic, here("Output", "models", "reproduction",  "mv_rep_ctemp2_remove_aquatic.rds"))
}else{
  meta_trait_treat2_remove_aquatic  <- readRDS(here("Output", "models", "reproduction", "mv_rep_ctemp2_remove_aquatic.rds"))
}
```
...
#| tbl-cap: Non-linear effects of temperature on mean effect size of reproduction (Sensitivity analysis removing aquatic species)
