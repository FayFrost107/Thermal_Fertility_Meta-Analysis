...
The relationship between reproduction and temperature depended on whether both males and females were exposed to experimental temperatures, or whether just females were exposed to experimental temperatures (*p*-value = `r p_value(meta_treat_sex$pval[6])`) (Interaction between the quadratic effect of (centered) temperature and sex exposed category (@tbl-sex). 
The samples sizes for the Both and Females only category are `r length(which(new_data$Sex.exposed == "Both"))` and `r length(which(new_data$Sex.exposed == "Female"))`  respectively.
...
We found that there was a significant difference in shape of the quadratic response to temperature those treated as immature individuals and those treated only as adults (*p*-value = `r p_value(meta_treat_ls$pval[6])`, Table (@tbl-ls). As is consisitent with our apriori beliefs, the fitted model predicts that immature individuals respond negativly to temperature away from 25C at a sharper and more pronounced rate.  The samples sizes for the Immature and Adult only categories are `r length(which(ls_data$Life.stage.of.animal == "Immature"))` and `r length(which(ls_data$Life.stage.of.animal == "Adult"))`  respectively.")  
...
saveRDS(meta_trait_treat6, here("Output", "models", "reproduction", "mv_rep_ctemp6.rds"))
  ...
  # treat temp^7 centered
meta_trait_treat7 <- rma.mv(es, VCV_shared,  mod= ~poly(c_treattemp, degree=7, raw=TRUE), random= list(~ 1|study_code,  ~1|obs), data= rdata, method= "REML", verbose=TRUE)
saveRDS(meta_trait_treat7, here("Output", "models", "reproduction", "mv_rep_ctemp7.rds"))


  # treat temp^8 centered
...
saveRDS(meta_trait_treat8, here("Output", "models", "reproduction", "mv_rep_ctemp8.rds"))
  ...
  meta_trait_treat6  <- readRDS(here("Output", "models",  "reproduction", "mv_rep_ctemp6.rds"))
  meta_trait_treat7  <- readRDS(here("Output", "models",  "reproduction", "mv_rep_ctemp7.rds"))
  meta_trait_treat8  <- readRDS(here("Output", "models",  "reproduction", "mv_rep_ctemp8.rds"))
}
