...
if(rerun){
  ...
  mlma_pest.only <- rma.mv(es, V2,  mod = ~poly(c_treattemp, degree=3, raw=TRUE), 
...
# re-run cubic model
meta_sa_exposure <- rma.mv(es, VCV_shared_sa,  mod= ~poly(c_treattemp, degree=3, raw=TRUE), 
                           random=list(~ 1|study_code,  ~1|obs), data= sdata, method= "REML")
