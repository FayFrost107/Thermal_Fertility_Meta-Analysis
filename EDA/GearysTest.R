...
warmdata <- read.csv("EDA/Survival project effect size props reftemp FF warm.csv")
...
calculate_smd <- function(rtemp1, rprop1, rani1, rn1, temp2, prop2, ani2, n2){
  ...
  Smd <- cbind(smd, smd_v, temp2-rtemp1, rtemp1, temp2, calculate_g(temp2, mean2, sd2, n2))   ############How would we calculate g for proportion data?
  return(Smd)
}
...
write.csv(cleaned_df, "Data/Gearys_propstest_data.csv")
